...
# The Kaltura channel that holds the whole line-up; every client reads it.
ALL_CHANNELS_ID = 346867
...
def list_channels_edit(label):
    ...
    if len(channels_list) > 0:
        for number in sorted(channels_list.keys()):
            ...
            list_item.setArt({'thumb': logos.logo_for(channels_list[number]), 'icon': logos.logo_for(channels_list[number])})
...
def edit_channel(id):
    ...
    if len(new_num) > 0 and int(new_num) > 0:
        ...
        if int(new_num) in channels_nums:
            xbmcgui.Dialog().notification('Vodafone TV', 'Číslo kanálu ' + new_num +  ' je použité u kanálu ' + channels_nums[int(new_num)]['name'], xbmcgui.NOTIFICATION_ERROR, 5000)
        else:  
...
def change_channels_numbers(from_number, direction):
    ...
    if len(change) > 0:
        ...
        if change > 0:
            ...
            xbmcgui.Dialog().notification('Vodafone TV', 'Je potřeba zadat číslo větší než jedna', xbmcgui.NOTIFICATION_ERROR, 5000)
    else:  
        xbmcgui.Dialog().notification('Vodafone TV', 'Je potřeba zadat číslo větší než jedna!', xbmcgui.NOTIFICATION_ERROR, 5000)
...
def list_channels_list_backups(label):
    ...
    if len(backups) > 0:
        ...
        xbmcgui.Dialog().notification('Vodafone TV', 'Neexistuje žádná záloha', xbmcgui.NOTIFICATION_INFO, 5000)          
...
def add_channel_group(label):
    ...
    group = input.getText()
    if len(group) == 0:
        xbmcgui.Dialog().notification('Vodafone TV', 'Je nutné zadat název skupiny', xbmcgui.NOTIFICATION_ERROR, 5000)
        sys.exit()          
    channels_groups = Channels_groups()
    if group in channels_groups.groups:
        xbmcgui.Dialog().notification('Vodafone TV', 'Název skupiny je už použitý', xbmcgui.NOTIFICATION_ERROR, 5000)
        sys.exit()          
    ...
    if group in channels_groups.channels:
        for channel in channels_groups.channels[group]:
            if channel in channels_list:
                ...
                list_item.setArt({'thumb': logos.logo_for(channels_list[channel]), 'icon': logos.logo_for(channels_list[channel])})
...
def select_channel_group(group):
    ...
    if (not group in channels_groups.channels or len(channels_groups.channels[group]) == 0) and group != 'all':
        xbmcgui.Dialog().notification('Vodafone TV', 'Vybraná skupina je prázdná', xbmcgui.NOTIFICATION_WARNING, 5000)    
...
class Channels:
    ...
    def get_channels(self):
        ...
        channels = {}
        session = Session()
        # 346867 is the "all channels" Kaltura channel -- the list every client
        # builds its line-up from.
        post = {"language":"ces","ks":session.ks,"filter":{"objectType":"KalturaChannelFilter","kSql":"","idEqual":ALL_CHANNELS_ID},"pager":{"objectType":"KalturaFilterPager","pageSize":300,"pageIndex":1},"clientTag":clientTag,"apiVersion":apiVersion}
        ...
        for channel in result:
            if 'Channel number' in channel['metas']:
                ...
                media_file = next((file for file in channel['mediaFiles'] if file.get('objectType') == 'KalturaMediaFile' and file.get('type') == 'DASH_AVC_FULLHD_HTTPS'), None)
                if media_file is not None:
                    file_url = media_file.get('url')
                    file_id = media_file.get('id')
                    
                    channels.update({int(channel['id']) : {'channel_number' : int(channel['metas']['Channel number']['value']), 'vdf_number' : int(channel['metas']['Channel number']['value']), 'name' : channel['name'], 'id' : channel['id'], 'externalIds' : channel['externalIds'], 'logo' : image, 'fileUrl' : file_url, 'fileId' : file_id, 'visible' : True}})
        return channels
    ...
    def reset_channels(self):
        ...
        if response == True:
            ...
            self.valid_to = -1
            self.load_channels()
            xbmcgui.Dialog().notification('Vodafone TV', 'Seznam kanálů byl resetovaný', xbmcgui.NOTIFICATION_INFO, 5000)
        else:
            self.valid_to = -1
            self.merge_channels()
            self.save_channels()
            xbmcgui.Dialog().notification('Vodafone TV', 'Seznam kanálů byl aktualizovaný', xbmcgui.NOTIFICATION_INFO, 5000)
    ...
    def restore_channels(self, backup):
        if os.path.exists(backup):
            try:
                ...
                if error.errno != 2:
                    xbmcgui.Dialog().notification('Vodafone TV', 'Chyba při načtení zálohy', xbmcgui.NOTIFICATION_ERROR, 5000)
            if data is not None:
                try:            
                    data = json.loads(data)
                    if 'valid_to' in data:
                        ...
                        try:
                            with codecs.open(filename, 'w', encoding='utf-8') as file:
                                file.write('%s\n' % data)
                                xbmcgui.Dialog().notification('Vodafone TV', 'Seznam kanálů byl obnovený', xbmcgui.NOTIFICATION_INFO, 5000) 
                        except IOError:
                            xbmcgui.Dialog().notification('Vodafone TV', 'Chyba uložení kanálů', xbmcgui.NOTIFICATION_ERROR, 5000)      
                except:
                    xbmcgui.Dialog().notification('Vodafone TV', 'Chyba při načtení zálohy', xbmcgui.NOTIFICATION_ERROR, 5000)
        else:
            xbmcgui.Dialog().notification('Vodafone TV', 'Záloha nenalezena', xbmcgui.NOTIFICATION_ERROR, 5000)      
    ...
    def save_channels_groups(self):
        ...
        if(len(self.groups)) > 0:
            try:
                ...
                xbmcgui.Dialog().notification('Vodafone TV', 'Chyba uložení skupiny', xbmcgui.NOTIFICATION_ERROR, 5000)      
        else:
            if os.path.exists(filename):
                os.remove(filename) 
