...
def list_archive(label):
    ...
    # Same rule as the live listing: leave out channels the household is not
    # subscribed to when "hide unsubscribed" is on.
    ...
    for number in sorted(channels_list.keys()):
        if hide_unsubscribed and entitlement.is_unentitled(
                channels_list[number]['id'], channels_list[number].get('fileId'), statuses):
            continue
        cnt += 1
